...
from deeprl.common.buffers import Memory
...
class DQN:
    def __init__(self, kwargs):
        ...
        self.q = Network(
            kwargs["net_design"]
        )  # use for action selection (nograd) and learning(grad)
    ...
    def choose_action(self, state):
        if np.random.rand() < self.epsilon:
            ...
            )  # now passing through network, so should be float really

            self.q.eval()
            # convert to numpy so you can do random choice of max vals
            q_vals = (
                self.q(state).cpu().detach().numpy().squeeze()
            )  # (1, num_ac) do i need to detach given that i have already no_grad?
            self.q.train()
    ...
    def _q_update(self):
        ...
        with torch.no_grad():
            ...
            max_tgt_net_vals = torch.max(nxt_tgt_net_vals, dim=1)[
                0
            ]  # shape: (m_batch,)
