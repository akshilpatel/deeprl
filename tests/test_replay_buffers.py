...
def test_sample():
    ...
    for i in range(len(states)):
        assert agent.env.observation_space.contains(states[i].cpu().detach().numpy()), (
            states[i].cpu().detach().numpy()
        )
        assert agent.env.observation_space.contains(
            next_states[i].cpu().detach().numpy()
        ), (next_states[i].cpu().detach().numpy())
